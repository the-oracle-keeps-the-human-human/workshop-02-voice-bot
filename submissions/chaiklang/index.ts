/**
 * maw chaiklang voice — Workshop 02 (scope A: TTS-first).
 * One-shot command that drives the persistent voice-daemon.mjs.
 *
 *   maw chaiklang voice join <guildId> <channelId>   spawn daemon → join + greet
 *   maw chaiklang voice say "<text>"                 queue text → daemon speaks
 *   maw chaiklang voice status                        is the daemon running?
 *   maw chaiklang voice leave                         stop daemon (leave channel)
 */
...
export const command = { name: "chaiklang", description: "ChaiKlang voice bot (Workshop 02)." };
...
export default async function handler(ctx: InvokeContext): Promise<InvokeResult> {
  ...
  if (args[0] !== "voice") { log("maw chaiklang voice <join|say|status|leave>"); return done(true); }
  const sub = args[1];

  switch (sub) {
    case "join": {
      const [guildId, channelId] = [args[2], args[3]];
      if (!guildId || !channelId) { log("usage: maw chaiklang voice join <guildId> <channelId>"); return done(false); }
      ...
      log(`🎙️ ChaiKlang กำลังเข้าห้อง voice ${channelId} (daemon pid ${child.pid})`);
      return done(true);
    }
    case "say": {
      const text = args.slice(2).join(" ").trim();
      if (!text) { log("usage: maw chaiklang voice say \"<text>\""); return done(false); }
    ...
    case "leave": {
      ...
      try { execFileSync("kill", [String(pid)]); } catch {}
      if (existsSync(PID)) unlinkSync(PID);
      log("👋 ChaiKlang ออกจากห้อง voice แล้ว");
      return done(true);
    }
    default:
      log("maw chaiklang voice <join|say|status|leave>");
      return done(true);
  }
}
