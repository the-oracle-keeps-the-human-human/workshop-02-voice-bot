...
Workshop 02 สอนเรื่อง Voice Bot — เอา bot เข้าห้องเสียง Discord ให้พูดได้ ซับซ้อนกว่า text bot มากเพราะต้องจัดการ persistent connection, audio pipeline, และ process lifecycle ทั้งหมด
...
- **async execFile ไม่ใช่ Sync** — ถ้าใช้ `execFileSync` ก็จะ block 20ms Opus loop ทำให้เสียง warp
...
วิธีแก้ก็เปลี่ยนไปใช้ Google TTS API (ผ่าน google-tts-api npm) ตัว endpoint มาจาก Google Translate เสถียรกว่า แล้วก็เปลี่ยน output เป็น OGG/Opus ไม่ต้องพึ่ง JS opus encoder อีกด้วย
...
curl dumps ข้อมูลทั้งไฟล์เข้ามาทีเดียว ถ้า buffer เล็ก player ก็จะ drain เร็วกว่า input เกิด underflow แล้วค้าง Idle หยุดเล่น พอตั้ง highWaterMark 96MB ก็ buffer ทั้งไฟล์ได้ เล่นต่อเนื่อง
...
ถ้าส่ง text แบบ `--voice evil` เข้า ffmpeg ตรง ก็จะถูกอ่านเป็น flag ได้ เติม space ข้างหน้าก็ป้องกันได้
...
### ผิดพลาดที่ 1: เชื่อ workshop example ไม่เช็คซอร์ส

ตอนทำ Workshop 01 คัดลอก `api.command()` จาก workshop guide ของ Atlas ไปใช้ตรง แต่ maw-js จริงใช้ `InvokeContext` / `InvokeResult` pattern พอรันก็ error "api.command is not a function" ทันที

**บทเรียน:** อ่าน source code ของ runtime ก่อนเสมอ ไม่ใช่แค่ copy ตัวอย่าง

### ผิดพลาดที่ 2: model ผิดใน CLAUDE.md และ BOOK.md

เขียนว่าใช้ "Opus 4.8 (1M context)" แต่จริงใช้ Opus 4.6 ข้อมูลเก่าจาก session ก่อน copy มาไม่ได้ตรวจสอบ

**บทเรียน:** ข้อมูลเกี่ยวกับตัวเองต้องเช็คกับ settings จริง อย่าเชื่อ memory อย่างเดียว

### ผิดพลาดที่ 3: ใช้ `node` แทน `bun` ใน index.ts

`spawn("node", [DAEMON])` จะพังเพราะ node หา bun global modules ไม่เจอ VPS นี้ลง dependencies ผ่าน bun ก็ต้องใช้ bun เป็น runtime
...
`.claude/` มี access.json ที่มี Discord bot token อยู่ แต่ไม่ได้เพิ่มใน .gitignore ถ้า commit ไป token ก็จะหลุดเข้า git history
